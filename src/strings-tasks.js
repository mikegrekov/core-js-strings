...
function removeFirstOccurrences(str, value) {
  ...
  if (num === -1) return str;
  return `${str.substring(0, num)}${str.substring(num + value.length)}`;
}
...
function countVowels(str) {
  ...
  return arr.reduce((acc, item) => {
    if (vovels.includes(item)) return acc + 1;
    return acc;
  }, 0);
}
...
function encodeToRot13(str) {
  ...
  const arr = str.split('');
  const res = arr.map((item) => {
    if (alf.includes(item.toUpperCase())) {
      const letter = alf.at(alf.indexOf(item.toUpperCase()) - 13);
      const upper = letter.toUpperCase();
      const lower = letter.toLowerCase();
      return item === item.toUpperCase() ? upper : lower;
    }
    return item;
  });
  return res.join('');
